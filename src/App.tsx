...
function App() {
  ...
  const plant = useMemo(() => {
    ...
    return u?.plants.find((p) => p.id === opened.plantId) || null;
  }, [opened, users]);
  ...
  function openByCode() {
    ...
    if (me?.role === "admin") {
      for (const u of users) {
        const p = u.plants.find((x) => x.code.toUpperCase() === codeInput.trim().toUpperCase());
        if (p) return openPlantBy(u.name, p.id);
      }
    } else if (me) {
      const p = me.plants.find((x) => x.code.toUpperCase() === codeInput.trim().toUpperCase());
  ...
  async function addEntry(file: File | null, note: string) {
    ...
    const e: Entry = { id: uid(10), date: now(), note, image: url };
    p.entries.unshift(e);
    p.cover = url;
    updateUser({ ...u });
  }
